...
ptr pred (ptr x) {return x == S.begin () ? x = S.end (), 0 : 0, --x; }
ptr next (ptr x) {return ++x, x == S.end () ? x = S.begin (), 0 : 0, x; }
...
void insert (crd x)
{
  crd R = *--S.end () < x ? S.begin () : lower_bound (S.begin (), S.end (), x), L = pred (R);
  if (turn (L, x, R) <= 0) return ;

  crd t;
  for ( ; turn (t = pred (L), x, L) > 0; L = t)
    S.erase (L);
  for ( ; turn (t = next (R), x, R) < 0; R = t)
    S.erase (R);
  S.insert (x);
}
...
int main ()
{
  ...
  for (int i = 0; i < 3; ++i)
    p[i].alpha = atan2 (p[i].y * 3 - my, p[i].x * 3 - mx), S.insert (p[i]);
  
  for (q -= 3; q--; )
    {
      // cin >> cmd >> x >> y;
      scanf ("%d%d%d", &cmd, &x, &y);
      crd p (x, y); p.alpha = atan2 (y * 3 - my, x * 3 - mx);
      if (cmd == 1) insert (p);
